...
*/
ArrayList<PVector> trackClicks; //for keeping track of click locations in generation mode

PVector jointDemo[]; //joints for the demo
PVector joints[]; //joints for normal mode
PVector manyJoints[]; //joints for long mode
PVector generate[]; //joints for generation mode
PVector branchJoints[]; //joints for branch mode
PVector branchExtra[]; //extra blue joints in branch mode
PVector tentJoints[]; //joints in tent mode
PVector member[]; //joints in mob mode
PVector opponent[]; //joints in pong mode

EndEffector endPointDemo; //end point for demo
EndEffector endPoint; //end point for normal mode and most other modes
EndEffector branchEnds[]; //extra end points for branching mode
EndEffector branchExtraEnd; //blue end point in branch mode
EndEffector tentEnd[]; //end points in a circle around the real end point for tent mode
EndEffector mobEnd; //the end point you drag around in mob mode
EndEffector mobReal[]; //the adjusted end points that the mob follows
EndEffector opponentPaddle; //end point that the IK system uses to hit the ball in pong mode

Chain IKDemo; //our static chain for the demo
Chain IK; //normal chain for normal mode and some others
Chain longChain; //chain for our long mode
Chain g; //our chain generated by the user
Chain branch[]; //Chains in branch mode
Chain bExtra; //extra chain in branch mode
Chain tentChain[]; //all the chains for our tent
Chain mob[]; //chains that control our mob
Chain right; //chain for our opponent in pong




float mobAngle; //angle for setting up new end effectors in mob mode

Pong game; //our pong game
...
//checks which mode we're currently in and performs the correct action for that mode
...
//look at mouse clicks for generation mode
void mouseClicked() {
...
//adjust end point in follow mode
...
//create chain when our generated points
void finalize() {
...
//remove points from our generated chain
void wipe() {
  PVector temp;
